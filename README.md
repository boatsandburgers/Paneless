...
- **Open in Claude Code:** opens a new terminal session in the workspace through Claude Code's own `claude-cli://open` link, with the prompt pre-filled and not sent (Claude Code shows "Prompt from an external link" until you press Enter). Context is also copied. Prompts over 5,000 characters open the workspace only; paste the copied context. Needs Claude Code's link handler, which registers itself after the first prompt of an interactive `claude` session on that machine.
